...
mysql.connections = mysql2.createPool({
    host     : 'localhost',
    user     : 'root',
    password : '',
    database : 'rostik'
...
mysql.connections.getConnection(function (err){
    if(err){
        switch(err.code){
    ...
    switch(state){
        case 0:{ // auth
            if(loggedAccount){
                ...
                mysql.connections.query('SELECT `password`,`id` FROM `accounts` WHERE `username` = ?', [username], function(err, res){
                    if(res.length > 0){
        ...
        case 1:{ // register
            if(username.length >= 3 && pass.length >= 5){
                mysql.connections.query('SELECT * FROM `accounts` WHERE `username` = ?', [username], function(err, res){
                    if(res.length > 0){player.call("loginHandler", ["takeninfo"])}
                    else{
                        bcrypt.hash(pass, null, null, function(err, hash) {
                            if(!err){
                                mysql.connections.query('INSERT INTO `accounts` SET username = ?, email = ?, password = ?, socialclub = ?, ip = ?', [username, 0, hash, player.rgscId, player.ip], function(err, res){
                                    if(!err){
                                        player.name = username;
                                        player.data.login = username;
...
mp.events.add("create:charselector", (player, firstName,lastName) => {
    mysql.connections.query('SELECT * FROM characters WHERE firstname=? AND lastname=?', [firstName,lastName], function(err, chars){
        if(!chars.length > 0 && !err){
            var xyz = "{'x':15,'y':15,'z':71,'r':0.0}";
            mysql.connections.query('INSERT INTO `characters` SET socialclub=?,adminlvl=?,xyz=?,firstname=?,lastname=?,health=?,licenses=?,bank_num=?,organization=?,accid=?', [player.rgscId,0,xyz,firstName,lastName,100,0,0,0,player.data.id], function(err, res){
                if(!err)
                {
