...
module.exports =
{
    ...
    saveAccount: function(player){
        ...
        mysql.connections.query('UPDATE `characters` SET xyz=?,lvl=?,exp=?,health=?,money=?,bank=? WHERE id = ?', [JSON.stringify(xyz),player.data.lvl,player.data.exp,player.health,player.data.money,player.data.bank, player.data.charid], function(err, res, row){
    ...
    loadAccount: function(player){
        ...
        player.call("create:camera", []);
        player.call("open:charselection", []);
        mysql.connections.query('SELECT adminlvl,firstname,lastname,lvl,exp,health,id,organization FROM characters WHERE accid = ?', [player.data.id], function(err, res, row){
            if(res.length){
                res.forEach(function(playerData){
                    mysql.connections.query('SELECT * FROM customization WHERE personid = ?', [playerData.id], function(err, custom, row){
                        if(custom.length){
    ...
    charenterAccount: function(player, id){
        mysql.connections.query('SELECT * FROM customization WHERE id=?', [id], function(err, custom){
            if(custom.length){
                custom.forEach(function(dat){

                    mysql.connections.query('SELECT * FROM characters WHERE id=?', [dat.personid], function(err, chars){
                        if(chars.length){
